-- =============================================================================
-- BarberHub Pro — db_completo.sql
-- Database COMPLETO: enums, tabelle, indici, RPC, RLS, trigger, permessi
-- =============================================================================
-- Eseguibile da Supabase SQL Editor su DB nuovo o esistente.
-- Idempotente: IF NOT EXISTS / CREATE OR REPLACE / DROP POLICY IF EXISTS.
--
-- PREREQUISITI:
--   • Progetto Supabase attivo (Postgres 15+)
--   • Schema auth presente (auth.users)
--
-- DOPO L'ESECUZIONE:
--   1. Settings → API → Reload schema cache
--   2. Configura NEXT_PUBLIC_SUPABASE_URL, NEXT_PUBLIC_SUPABASE_ANON_KEY
--      e SUPABASE_SERVICE_ROLE_KEY in .env.local
-- =============================================================================
...
-- Profili utente (legati a auth.users)
...
-- Slot pubblici (disponibilità per prenotazioni online)
...
-- =============================================================================
-- MIGRAZIONI DATI (solo su DB esistente, safe su DB nuovo)
-- =============================================================================

-- Sede di default per aziende senza sedi
...
-- Backfill location_id su righe esistenti
update public.clients cl
set location_id = coalesce(
  cl.location_id,
  (select l.id from public.locations l where l.company_id = cl.company_id order by l.created_at asc limit 1)
)
...
set location_id = coalesce(
  p.location_id,
  (select l.id from public.locations l where l.company_id = p.company_id order by l.created_at asc limit 1)
)
...
set location_id = coalesce(
  pr.location_id,
  (select l.id from public.locations l where l.company_id = pr.company_id order by l.created_at asc limit 1)
)
where pr.role in ('MANAGER', 'BARBER') and pr.company_id is not null and pr.location_id is null;

-- Not null su location_id dopo backfill
...
-- Unique constraint su clienti (stesso telefono = stesso cliente, per sede)
...
create index if not exists idx_location_open_slots_company on public.location_open_slots (company_id);
...
create index if not exists idx_appointments_company_location_date_status
  on public.appointments (company_id, location_id, date, status);
create index if not exists idx_payments_company_date on public.payments (company_id, date);
create index if not exists idx_payments_company_date_method
  on public.payments (company_id, date, method) where method is not null;
create index if not exists idx_location_open_slots_loc_date on public.location_open_slots (location_id, slot_date);
create index if not exists idx_location_open_slots_comp_loc_date_mins
  on public.location_open_slots (company_id, location_id, slot_date, slot_mins);
create index if not exists idx_companies_name on public.companies (name);

-- Unique index per slot
...
-- =============================================================================
-- TRIGGER: profilo automatico su nuovi utenti Auth
-- =============================================================================
...
-- Helper RLS: solo authenticated (serve per le policy RLS)
grant execute on function public.is_super_admin() to authenticated;
grant execute on function public.my_company_id() to authenticated;
grant execute on function public.my_role() to authenticated;
grant execute on function public.my_location_id() to authenticated;
grant execute on function public.is_company_admin_or_manager() to authenticated;
...
-- Revoca esplicitamente da anon (sicurezza)
revoke all on function public.is_super_admin() from public, anon;
revoke all on function public.my_company_id() from public, anon;
revoke all on function public.my_role() from public, anon;
revoke all on function public.my_location_id() from public, anon;
revoke all on function public.is_company_admin_or_manager() from public, anon;
...
-- RPC pubbliche: anon + authenticated
grant execute on function public.rpc_public_book_appointment(uuid, text, text, text, text, timestamptz, int, uuid, uuid) to anon, authenticated;
grant execute on function public.rpc_public_availability(uuid, uuid, date, int) to anon, authenticated;
grant execute on function public.rpc_public_company_info(uuid) to anon, authenticated;
grant execute on function public.rpc_public_client_bookings(uuid, uuid, text, text) to anon, authenticated;
